...
class _LayoutPageState extends State<LayoutPage> {
  ...
  Widget build(BuildContext context) {
    ...
    // Use DashboardBloc from app level
          ...
          builder: (context, state) {
            return Stack(
              children: [
                ...
                if (isLoading) _FullScreenSkeleton(colors: colors),
              ],
            );
          },
        );
...
// ═══════════════════════════════════════════════════════════════════════════
// AURA BOTTOM NAV — floating glassy pill with FAB
// ═══════════════════════════════════════════════════════════════════════════
...
class _AuraBottomNav extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    const navHorizontalMargin = 24.0;
    final navWidth = screenWidth - (navHorizontalMargin * 2);

    // Small margin above home indicator, or fixed margin if no indicator
    final navBottomMargin = bottomPad > 0 ? 4.0 : 12.0;

    return SizedBox(
      height: navHeight + bottomPad + 26, // Extra space for FAB overhang
      child: Stack(
        ...
        children: [
          ...
          Positioned(
            bottom: bottomPad + navBottomMargin,
            child: ClipRRect(
              ...
              child: BackdropFilter(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: colors.navBackground.withValues(alpha: 0.45),
                    borderRadius: BorderRadius.circular(32),
                    border: Border.all(
          ...
          // ── Camera FAB — floats above center ──────────────────────
          Positioned(
            bottom: bottomPad + navBottomMargin + navHeight - 26,
